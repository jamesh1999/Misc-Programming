            ...
            if self._frame_ready:
                ...
                for part in sorted(dist_list,key=itemgetter(1),reverse=True):
                    ...
                    if part[2]=="f" and self.displayFaces:
                        centre=(sum([node.x for node in partobj.corners]) / 3,sum([node.y for node in partobj.corners]) / 3,sum([node.z for node in partobj.corners]) / 3)
                        ...
                        if facedist<0:
                            ...
                            for lightobj in self.lights:
                                ...
                                if partobj.spec_shine/2 == int(partobj.spec_shine/2):
                                    dotvar=max(dot**partobj.spec_shine*(-1 if dot>=0 else 1),0)
                                else:
                                    dotvar=max(-dot**partobj.spec_shine,0)
                                ...
                                if self.displayRays:
                                    try:
                                        ...
                                        if a_zz<0 or b_zz<0:
                                            continue;
                                        a_x=(centre[0]-j.x)/a_zz*d+self.width/2
                                        a_y=(centre[1]-j.y)/a_zz*d+self.height/2

                                        b_x=(centre[0]+lvector[0]-j.x)/b_zz*d+self.width/2
                                        b_y=(centre[1]+lvector[1]-j.y)/b_zz*d+self.height/2
                                    ...
                                    try:
                                        ...
                                        b_x=(centre[0]+rvector[0]-j.x)/b_zz*d+self.width/2
                                        b_y=(centre[1]+rvector[1]-j.y)/b_zz*d+self.height/2
                            ...
                            colour0=min(partobj.col[0]*(light[0]/255*(255-(self.min_brightness*self.min_colour[0]/100))+self.min_brightness*self.min_colour[0]/100)/255+tot_specularval[0],255)
                            colour1=min(partobj.col[1]*(light[1]/255*(255-(self.min_brightness*self.min_colour[1]/100))+self.min_brightness*self.min_colour[1]/100)/255+tot_specularval[1],255)
                            colour2=min(partobj.col[2]*(light[2]/255*(255-(self.min_brightness*self.min_colour[2]/100))+self.min_brightness*self.min_colour[2]/100)/255+tot_specularval[2],255)
                            ...
                            if self.displayNorms:
                                try:
                                    ...
                                    b_x=(partobj.norm[0]+centre[0]-j.x)/b_zz*d+self.width/2
                                    b_y=(partobj.norm[1]+centre[1]-j.y)/b_zz*d+self.height/2
                                except ZeroDivisionError:
                                    continue;
                ...
                if self.displayLights:
                    for light in self.lights:
                        try:
                            ...
                            if zz<0:
                                    continue;
                            x=(light.x-j.x)/zz*d+self.width/2
                            y=(light.y-j.y)/zz*d+self.height/2
                        except ZeroDivisionError:
                            continue;
...
def main():
    ...
    while True:
        ...
        if choice!="N" and choice!="n" and choice!="y" and choice !="Y":
            ...
            cube_frame.add_faces((((0,1,3,2),white),((6,7,5,4),cyan),((0,2,5,7),red),((6,4,3,1),green),((7,6,1,0),blue),((2,3,4,5),yellow)))
            objects={}
            objects["cube"]=cube
