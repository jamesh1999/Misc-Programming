                ...
                for key,obj in self.objects.items():
                    for i in obj.sub_objects:
                        for node in i.nodes:
                            ...
                            #Identify node->screen coords and distance from screen
                            node.camdist=getDistance((node.x,node.y,node.z),(j.x,j.y,j.z))
                            dist_list.append([node,node.camdist,"n"])
                            xx=node.x-j.x
                            yy=node.y-j.y
                            zz=node.z-j.z
                            rd=math.hypot(yy,xx)
                            ...
                            if zz<=0:
                                ...
                                continue;
                            node.renderx=int(xx/zz*d+self.width/2)
                            node.rendery=int(yy/zz*d+self.height/2)
