...
def convertInstruction(instruction):
	...
	if not instruction[0] in ["SIGNED", "UNSIGNED", "ASSEMBLER"]:

		unsigned, immediate = getInstructionType(instruction)

		#Switch to unsigned instructions if necessary
		if unsigned and instruction[0] in UNSIGNED_TYPE:
			if not instruction[0][-1] == "I":
				instruction[0] += "U"
			else:
				instruction[0] = instruction[0][:-1] + "UI"
		...
		REGISTER_STATUS[getRegister(instruction[2])] = instruction[0]
