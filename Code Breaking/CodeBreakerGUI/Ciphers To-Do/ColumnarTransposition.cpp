...
std::string decryptColumnar(unsigned keylen, const std::vector<unsigned>& key, const std::string& text)
{
	...
	for (unsigned i = 2; i < text.size(); ++i)
    {
		if (text.size() % i == 0)
		{
            ...
            for (unsigned iter = 0; iter < 10; ++iter)
            {
                ...
                parent_score = evaluate(decryptColumnar(i, parent_key, text));
                ...
                for (unsigned cnt = 0; cnt < std::min(i*i, 40 * i); ++cnt)
                {
					...
					//Evaluate new key
					double child_score = evaluate(decryptColumnar(i, child_key, text));
				...
				if (parent_score>top_score)
				{
                    ...
                    pWorker->setPlainTextOutput(QString::fromStdString(decryptColumnar(top_len, top_key, text)));
                }
            }
