...
std::string decryptPlayfair(const std::string& key, std::string text)
{
...
void playfair(std::string text)
{
	...
	for (double T = TEMP; T >= 0; T -= STEP)
	{
		for (unsigned cnt = 0; cnt < 674; ++cnt)
		{
			...
			double eval = evaluate(decryptPlayfair(child_key, text));
        ...
        pWorker->setPlainTextOutput(QString::fromStdString(decryptPlayfair(parent_key, text)));
	}
}
