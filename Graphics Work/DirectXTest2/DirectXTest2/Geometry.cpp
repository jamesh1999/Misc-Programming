...
void Geometry::GenerateCuboid(MeshData* mesh, float w, float h, float d)
{
	Vertex buff;
...
void Geometry::GenerateSphere(MeshData* mesh, float radius, int nSlices, int nSegments)
{
	//Sides
...
void Geometry::GenerateCylinder(MeshData* mesh, float topRadius, float bottomRadius, float h, int nSlices, int nSegments)
{
	//Sides
	...
	for(int i = 0; i <= nSlices; ++i)
	{
		...
		for (int j = 0; j <= nSegments; ++j)
		{
			...
			buff.tex = { theta * DirectX::XM_1DIV2PI, 0.25f + y / (h * 2) };
	...
	for (int j = 0; j < nSegments; ++j)
	{
		...
		buff.tex = { sinf(theta) / 4 + 0.25f, cosf(theta) / 4 + 0.75f};
	...
	for (int j = 0; j < nSegments; ++j)
	{
		...
		buff.tex = { sinf(theta) / 4 + 0.75f, cosf(theta) / 4 + 0.75f};
...
void Geometry::GenerateQuad(MeshData* mesh, float w, float l, int xSplit, int ySplit)
{
	//Vertices
	float origX = -w / 2;
	float origY = -l / 2;
	...
	for(int x = 0; x <= xSplit; ++x)
		for(int y = 0; y <= ySplit; ++y)
		{
			...
			buff.tex = { static_cast<float>(x) / xSplit, static_cast<float>(y) / ySplit };
