...
void Terrain::ApplyHeightmap(MeshData* mesh)
{
	...
	for(int x = 1; x < 100; ++x)
		for(int y = 1; y < 100; ++y)
		{
			DirectX::XMVECTOR x0 = DirectX::XMLoadFloat3A(&mesh->vertices[y + (x - 1) * 101].pos);
			DirectX::XMVECTOR x1 = DirectX::XMLoadFloat3A(&mesh->vertices[y + (x + 1) * 101].pos);
			DirectX::XMVECTOR y0 = DirectX::XMLoadFloat3A(&mesh->vertices[y - 1 + x * 101].pos);
			DirectX::XMVECTOR y1 = DirectX::XMLoadFloat3A(&mesh->vertices[y + 1 + x * 101].pos);

			DirectX::XMVECTOR dX = DirectX::XMVectorSubtract(x1, x0);
			DirectX::XMVECTOR dY = DirectX::XMVectorSubtract(y1, y0);

			DirectX::XMStoreFloat3A(&mesh->vertices[y + x * 101].normal,
...
void Terrain::GenerateGroundTexture(DirectX::XMFLOAT3** out, int x, int y, MeshData* in)
{
	//Create perlin noise
