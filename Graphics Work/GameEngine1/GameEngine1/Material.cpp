...
void Material::setTextureData(uint8_t* data, int w, int h)
{
	...
	setTextureData(vec, 1);
}

void Material::setTextureData(std::vector<TexData> textures, int arraySize)
{
	if(m_tex != nullptr)
	{
		m_tex->Release();
	...
	{
		texData[i].SysMemPitch = 4 * w_max * h_max;
		texData[i].SysMemSlicePitch = 4 * w_max;
