...
std::string Shader_Manager::ReadShader(const std::string& filename)
{
	...
	shaderCode.resize(static_cast<unsigned>(file.tellg()));
...
void Shader_Manager::CreateProgram(const std::string& shaderName, const std::string& vertexShaderFilename, const std::string& fragmentShaderFilename)
{
	...
	//Check for link errors and terminate
	...
	else
	{
		...
		std::cout << "Shader Loader : Program " << shaderName.c_str() << " already defined" << std::endl;
		std::terminate();
	}
}
...
const GLuint Shader_Manager::GetShader(const std::string& shaderName)
{
	...
	if (it == programs.end())
	{
		...
		std::cout << "Shader Loader : Program " << shaderName.c_str() << " not defined" << std::endl;
		std::terminate();
	}
	else
	{
...
const GLuint Shader_Manager::RemoveShader(const std::string& shaderName)
{
